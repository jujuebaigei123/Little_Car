...
TGPS_Base::TGPS_Base()
{
    ...
    else if (m == "SER")
    {
        ...
        {
            char cmd[1000];
            sprintf(cmd,"sudo chmod 777 %s", devname.c_str());
            system(cmd);
...
int TGPS_Base::DataProc(const char *buf)
{
    ...
    if (res) //  必须有角度
    {
       ...
       msg.raw_Angle = 270-gdata.Angle;// - 90;
        ...
        // msg.map_x = msg.raw_map_x;
        // msg.map_y = msg.raw_map_y;
        // printf("raw_map_x:%.2f,map_x:%.2f,raw_map_y:%.2f,map_y:%.2f\n",msg.raw_map_x,msg.map_x,msg.raw_map_y,msg.map_y);
