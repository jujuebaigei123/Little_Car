...
{   
    ...
    // static geometry_msgs::PoseStamped last_pose=pose;
    // static float wu_angle=0;
    // float wu_d=0;
    // wu_d=P2P(last_pose.pose.position,pose.pose.position);
    // if(wu_d>0.01)
    // {
    //     wu_angle=-atan2(msg->map_x-last_pose.pose.position.x,msg->map_y-last_pose.pose.position.y)+M_PI/2;
    //     //  printf("last_posex:%.2f,last_posey:%.2f\n",last_pose.pose.position.x,last_pose.pose.position.y);
    //     // printf("current_posex:%.2f,current_posey:%.2f\n",msg->map_x,msg->map_y);
    //     last_pose.pose.position.x=msg->map_x;
    //     last_pose.pose.position.y=msg->map_y;
    // }
    // printf("wu_angle:%.2f\n",wu_angle);
    // pose.pose.orientation=tf::createQuaternionMsgFromYaw(wu_angle);
    // printf("angle:%.2f",msg->Angle);
