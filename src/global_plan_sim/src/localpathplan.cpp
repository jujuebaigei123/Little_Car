...
private:
    ...
    nav_msgs::Path local_path;  //  从global_path中抽取的局部路径  zhang1222222222222222222
...
void TLocalPathPlan::SpeedPlan(nav_msgs::Path &path, float min_vel, float safe_dis)
{
    ...
    // else  
    //  no jiasu
    path.poses.begin()->pose.position.z = path.poses[0].pose.position.z; //min_vel;
    ...
    for (int i = 1; i < path.poses.size(); i++)
    {
        ...
        if (vk > acc_per_len)
            ...
            vk = dec_per_len;
        // path.poses[i].pose.position.z = ref_vel;
